...
class Trajectory:
    ...
    def __check_gmxlib(self):
        ...
        if not p:
            print("pmx_Error_> Path to Gromacs shared libraries is not set (GMX_DLL)", file=sys.stderr)
            print("pmx_Error_> Cannot load \"libgmx.so\"", file=sys.stderr)
            sys.exit(1)
    ...
    def read_first_xtc(self):
        ...
        if not ret:
            print("pmx_Error_> in Trajectory.read_first_xtc()", file=sys.stderr)
            sys.exit(1)
        self.__have_first_frame = True

    def read_next_xtc(self):
        if not self.__have_first_frame:
            print("pmx_Error_> First frame not read", file=sys.stderr)
            sys.exit(1)
